...
import podoImg from '../assets/dedective_podo.png';
import { FORM_IDS, type FormKey } from '../constants/forms';
import { useUserForms } from '../hooks/useUserForms';

const PROJECT_FORM_IDS = new Set<string>(Object.values(FORM_IDS));
...
const LEADS: Lead[] = [
  {
    ...
    icon: '📍',
    title: 'Check-ins',
    hook: 'Trace last known locations and movements.',
    accent: 'var(--podo-blue)',
    tilt: '-rotate-2',
  },
  {
    key: 'SIGHTINGS',
    icon: '👁️',
    title: 'Sightings',
    hook: 'Hear what witnesses claim they saw.',
    accent: 'var(--podo-green)',
    tilt: 'rotate-1',
  },
  {
    key: 'MESSAGES',
    icon: '✉️',
    title: 'Messages',
    hook: 'Read intercepted communications.',
    accent: 'var(--podo-orange)',
    tilt: '-rotate-1',
  },
  {
    key: 'PERSONAL_NOTES',
    ...
    hook: 'Peek into private journal entries.',
    accent: 'var(--podo-yellow)',
    tilt: 'rotate-2',
  },
  {
    key: 'ANONYMOUS_TIPS',
    ...
    hook: 'Decode tips from confidential sources.',
    accent: '#a78bfa',
    tilt: '-rotate-1',
  },
];
...
export function HomePage() {
  ...
  const projectForms = useMemo(
    () => forms.filter((f) => PROJECT_FORM_IDS.has(f.id)),
    [forms]
  );

  const totalClues = projectForms.reduce(
    (sum, f) => sum + parseInt(f.count, 10),
    0
  );

  const liveCounts = useMemo(() => {
    const map = new Map<string, number>();
    projectForms.forEach((f) => map.set(f.id, parseInt(f.count, 10)));
    return map;
  }, [projectForms]);
  ...
  return (
    <main className="flex flex-1 flex-col">
        ...
        <div className="relative mx-auto grid w-full max-w-6xl items-center gap-12 px-6 pb-20 pt-16 lg:grid-cols-[1fr_1.15fr] lg:gap-16 lg:pb-28 lg:pt-24">
          ...
          <div className="text-center lg:text-left">
            ...
            <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-white/70 sm:text-lg lg:mx-0">
              Five forms. One mystery. Help Detective Podo connect the dots
              between check-ins, sightings, intercepted messages and anonymous
              tips — and bring the case home.
            </p>
            ...
            <div className="mt-12 grid grid-cols-3 gap-4 border-t border-white/10 pt-6 text-center lg:text-left">
              <Stat
                label="Total clues"
                value={isLoading ? '…' : totalClues.toString()}
              />
              <Stat label="Open leads" value="5" />
              <Stat
                label="JotForm sync"
                value={
        ...
        <div className="relative mx-auto w-full max-w-6xl px-6 py-20">
          <div className="mb-14 flex flex-col items-center text-center">
            ...
            <p className="mt-3 max-w-lg text-(--text)">
              Each card is a different angle on the case. Follow whichever
              thread feels hottest — you can always loop back.
            </p>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {LEADS.map((lead, i) => (
              <LeadCard
                key={lead.key}
                lead={lead}
                count={liveCounts.get(FORM_IDS[lead.key]) ?? null}
                isLoading={isLoading}
                index={i + 1}
              />
            ))}
      ...
      <section
        className="relative overflow-hidden"
        style={{ background: 'var(--podo-navy-light)' }}
      >
        <div
        ...
        <div className="relative mx-auto w-full max-w-6xl px-6 py-20">
          ...
          <div className="mt-14 flex flex-col items-center gap-4 rounded-3xl border border-white/10 bg-white/5 px-6 py-6 text-center backdrop-blur sm:flex-row sm:px-8 sm:text-left">
            ...
            <p className="flex-1 text-sm text-white/80 sm:text-base">
              Live data is fetched on every visit — no fixtures, no fakes. Just
              real evidence, straight from the JotForm API.
            </p>
            <Link
              to="/case"
...
function LeadCard({
  ...
  return (
    <Link
      to={`/case/${lead.key.toLowerCase()}`}
      className={`group relative flex flex-col gap-3 rounded-2xl border border-(--border) bg-white p-6 no-underline shadow-(--shadow-sm) transition-all duration-300 hover:-translate-y-1.5 hover:rotate-0 hover:shadow-(--shadow-lg) ${lead.tilt}`}
    >
      {/* Folder tab */}
      <span
      ...
      <div className="flex items-start justify-between gap-3 pt-2">
        <div
          ...
          style={{ background: `${lead.accent}1f` }}
        >
          {lead.icon}
        </div>
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-(--text)">{lead.hook}</p>
      </div>
      ...
      <div className="mt-2 flex items-end justify-between border-t border-dashed border-(--border) pt-3">
        ...
        <span
          ...
          style={{ background: `${lead.accent}1a`, color: lead.accent }}
        >
          Investigate
          <span aria-hidden="true">→</span>
        </span>
